...
router.get('/', requireAuth, requireRole('master', 'socio'), async (req, res) => {
  ...
  const visiveis = req.user.tipo === 'socio' ? usuarios.filter((u) => u.tipo !== 'master') : usuarios;
...
router.post('/', requireAuth, requireRole('master', 'socio'), async (req, res) => {
  const { nome, tipo, login, senha, oab, nacionalidade, estadoCivil, rg, cpf, ativo } = req.body || {};
  ...
  const novo = {
    ...
    rg: rg || '', cpf: cpf || '', vinculoId: null, clienteId: null,
  };
  usuarios.push(novo);
...
// Edita dados cadastrais (não a senha) de um sócio/associado
router.patch('/:id', requireAuth, requireRole('master', 'socio'), async (req, res) => {
  ...
  const campos = ['nome', 'oab', 'ativo', 'nacionalidade', 'estadoCivil', 'rg', 'cpf'];
